...
async def user_session(
	...
	for query_id in range(queries_per_user):
		...
		if one_rpups:
			# wait at least a second until the last query is processed
			await asyncio.sleep(max(0, 1 - (time.time() - last_query_time)))
...
async def smoke_test(args):
	...
	if use_dataset:
		...
		if args.single_run or args.same_text:
			article = get_random_article_from_dataset(dataset)
...
def main():
	...
	return asyncio.run(smoke_test(args))["failures"]
