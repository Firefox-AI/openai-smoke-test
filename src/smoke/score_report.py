...
def calculate_performance_score(
    value, good_threshold, bad_threshold, lower_is_better=True
):
    """
...
def stats_summary(values, name):
    ...
    p90 = (
        quantiles(values_sorted, n=10)[8]
        if len(values_sorted) >= 2
        else values_sorted[0]
    )
...
def check_thresholds(threshold_errors, threshold_config, stats_dict):
    for key, threshold in threshold_config.items():
        if (
            key in stats_dict
            and stats_dict[key][1] != "-"
            and float(stats_dict[key][1]) < threshold
        ):
...
def get_report(stats_path: str, summarization_config: dict):
    ...
    round_trip_stats = stats_summary(
        [s["total_time"] for s in stats if s.get("total_time")], "Round trip (s)"
    )

    all_rouge_stats = {
        "rouge1": stats_summary(
            [
                s["summary_score"]["rouge"]["rouge1"]
                for s in stats
                if s.get("summary_score", {}).get("rouge", {}).get("rouge1") is not None
            ],
            "Rouge 1",
        ),
        "rouge2": stats_summary(
            [
                s["summary_score"]["rouge"]["rouge2"]
                for s in stats
                if s.get("summary_score", {}).get("rouge", {}).get("rouge2") is not None
            ],
            "Rouge 2",
        ),
        "rougeLsum": stats_summary(
            [
                s["summary_score"]["rouge"]["rougeLsum"]
                for s in stats
                if s.get("summary_score", {}).get("rouge", {}).get("rougeLsum")
                is not None
            ],
            "RougeLsum",
        ),
    ...
    bleu_stats = stats_summary(
        [
            s["summary_score"]["bleu"]["bleu"]
            for s in stats
            if s.get("summary_score", {}).get("bleu", {}).get("bleu") is not None
        ],
        "BLEU",
    )
    all_unieval_stats = {
        "unieval_consistency": stats_summary(
            [
                s["summary_score"]["unieval"]["consistency"]
                for s in stats
                if s.get("summary_score", {}).get("unieval", {}).get("consistency")
                is not None
            ],
            "Unieval consistency",
        ),
        "unieval_coherence": stats_summary(
            [
                s["summary_score"]["unieval"]["coherence"]
                for s in stats
                if s.get("summary_score", {}).get("unieval", {}).get("coherence")
                is not None
            ],
            "Unieval coherence",
        ),
        "unieval_fluency": stats_summary(
            [
                s["summary_score"]["unieval"]["fluency"]
                for s in stats
                if s.get("summary_score", {}).get("unieval", {}).get("fluency")
                is not None
            ],
            "Unieval fluency",
        ),
        "unieval_relevance": stats_summary(
            [
                s["summary_score"]["unieval"]["relevance"]
                for s in stats
                if s.get("summary_score", {}).get("unieval", {}).get("relevance")
                is not None
            ],
            "Unieval relevance",
        ),
        "unieval_overall": stats_summary(
            [
                s["summary_score"]["unieval"]["overall"]
                for s in stats
                if s.get("summary_score", {}).get("unieval", {}).get("overall")
                is not None
            ],
            "Unieval overall",
        ),
    ...
    total_duration = sum(
        s["total_time"] for s in stats if s.get("total_time") is not None
    )
    ...
    if metrics_table[0][1] != "-" and float(metrics_table[0][1]) > metric_threshold.get(
        "ttft", float("inf")
    ):
        threshold_errors.append(
            {
    ...
    if metrics_table[1][1] != "-" and float(metrics_table[1][1]) < metric_threshold.get(
        "per_query_tps", float("-inf")
    ):
        threshold_errors.append(
            {
                "name": "PER QUERY TPS",
    ...
    if metrics_table[2][1] != "-" and float(metrics_table[2][1]) > metric_threshold.get(
    ...
    num_hiccups = sum(
        [
            s["summary_score"]["hiccup"]
            for s in stats
            if s.get("summary_score", {}).get("hiccup")
        ]
    )
    percentage_of_hiccups = num_hiccups / max(1, total)
    print(
        f"Num hiccups: {num_hiccups} --- Percentage of hiccups: {percentage_of_hiccups:.4f}"
    )
    weights = {
        "rouge1": 0.1,
        "rouge2": 0.1,
    ...
    rouge_score = sum(
        float(all_rouge_stats[key][1]) * weights[key]
        for key in all_rouge_stats
        if all_rouge_stats[key][1] != "-"
    )
    unieval_score = sum(
        float(all_unieval_stats[key][1]) * weights[key]
        for key in all_unieval_stats
        if all_unieval_stats[key][1] != "-"
    )
    overall_score = (
        rouge_score
    ...
    check_thresholds(
        threshold_errors, score_threshold_config.get("rouge", {}), all_rouge_stats
    )

    if bleu_stats[1] != "-" and float(bleu_stats[1]) < score_threshold_config.get(
        "bleu", 0
    ):
        threshold_errors.append(
            {
    ...
    check_thresholds(
        threshold_errors, score_threshold_config.get("unieval", {}), all_unieval_stats
    )
...
def score_reports():
    ...
    if not stats_files:
        raise FileNotFoundError(
            f"No .jsonl files found in {STATS_DIR} or its subdirectories"
        )
    ...
    for r in all_reports:
        ...
        round_trip_score = calculate_performance_score(
            round_trip,
            ROUND_TRIP_THRESHOLDS["good"],
            ROUND_TRIP_THRESHOLDS["bad"],
...
async def compare_with_ai():
    ...
    for idx in tqdm(indices[: min(LIMIT, len(reference_stats))]):
        ...
        for other_path, other_stats in all_other_stats:
            matching = next(
                (
                    e
                    for e in other_stats
                    if e.get("original_text") == ref_entry.get("original_text")
                ),
                None,
            )
            if not matching:
            ...
            m1 = (
                "mistral-small-2503"
                if shuffle
                else other_path.replace(STATS_DIR, "").replace(".jsonl", "")
            )
            m2 = (
                other_path.replace(STATS_DIR, "").replace(".jsonl", "")
                if shuffle
                else "mistral-small-2503"
            )
            s1 = (
                ref_entry.get("predicted_summary", "")
                if shuffle
                else matching.get("predicted_summary", "")
            )
            s2 = (
                matching.get("predicted_summary", "")
                if shuffle
                else ref_entry.get("predicted_summary", "")
            )
            prompt = (
            ...
            if count % 4 == 0:
                ...
                for i, content in enumerate(contents):
                    try:
                        ...
                        preferences.append(
                            {"model_1": m1, "model_2": m2, "preferred": preferred}
                        )
                    except Exception as e:
                        print(e)
                        m1, m2, _, _ = batch_info[i]
                        preferences.append(
                            {"model_1": m1, "model_2": m2, "preferred": "error"}
                        )
                batch_prompts = []
                batch_info = []
    ...
    preference_counter = Counter(
        [p["preferred"] for p in preferences if "preferred" in p]
    )
...
def score_ai_comparisons():
    ...
    with open(comparison_path, "r") as f:
        for line in f:
            ...
            if pref:
                preference_counts[
                    record.get("reference_model")
                    if pref == "reference"
                    else record.get("predicted_model")
                ] += 1
            total += 1

    # Show average predicted scores for each predicted model
    model_scores = collections.defaultdict(list)
    model_scores["mistral-2503-small"].append(
    ...
    for model, scores in model_scores.items():
        ...
        if model == "mistral-2503-small":
            mistral_pref_pct = (
                (dict(preference_counts).get("mistral-small-2503", 0) / total * 100)
                if total
                else 0
            )
    ...
    print(
        tabulate(
            table, headers=["Model", "Average Score", "Preferred %"], tablefmt="grid"
        )
    )
...
def score_ai_comparisons2():
    ...
    for model in sorted(model_counts.keys()):
        ...
        losses = sum(
            ...
            if (r.get("model_1") == model or r.get("model_2") == model)
            and r.get("preferred") not in [model, "no preference"]
        )
        no_pref_losses = sum(
            1
            for r in preferences
            if (r.get("model_1") == model or r.get("model_2") == model)
            and r.get("preferred") == "no preference"
        )
        table.append(
            [model, wins, losses, no_pref_losses, appearances, f"{win_pct:.2f}%"]
        )
    ...
    table_sorted = sorted(
        table, key=lambda x: float(x[5].replace("%", "")), reverse=True
    )
    print(
        tabulate(
            table_sorted,
