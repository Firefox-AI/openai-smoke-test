...
async def load_prompts(datasets_config, feature_datasets):
    ...
    print(
        f"INFO: Found a total of {total_prompts} prompts across {len(feature_datasets)} dataset(s)."
    )
    ...
    for dataset_name in feature_datasets:
        ...
        if dataset_type == "custom_messages_jsonl":
            ...
            print(
                f"WARNING: Skipping unsupported dataset type '{dataset_type}' for dataset '{dataset_name}'"
            )


async def worker(
    worker_id, client, prompt_generator, results_queue, semaphore, config, run_config
):
    """
        ...
        async with semaphore:
            ...
            try:
                ...
                if config.get("test_config", {}).get("bust_kv_cache", False):
                    probability = config["test_config"].get(
                        "bust_cache_probability", 100
                    )
                    if random.uniform(0, 100) < probability:
                        cache_bust_token = (
                            f"[IGNORE THIS CACHE BUST TOKEN: {uuid.uuid4()}]"
                        )
                        ...
                        if not system_message_found:
                            messages.insert(
                                0, {"role": "system", "content": cache_bust_token}
                            )
                ...
                if (
                    ...
                    if (
                        hasattr(last_chunk.usage, "prompt_tokens_details")
                        and last_chunk.usage.prompt_tokens_details is not None
                    ):
                        cached_tokens = last_chunk.usage.prompt_tokens_details.get(
                            "cached_tokens", -1
                        )
                ...
                result = {
                    ...
                    "user_tps": output_tokens / (end_time - start_time)
                    if (end_time - start_time) > 0 and output_tokens != -1
                    else 0,
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run a stress test on an OpenAI-compatible endpoint."
    )
    parser.add_argument(
        "--test-config", required=True, help="Path to the stress-test.yaml file."
    )
    parser.add_argument(
        "--feature", required=True, help="The feature to test (e.g., 'chatbot')."
    )
    parser.add_argument(
        "--vendor", required=True, help="The vendor to test (e.g., 'vertexai')."
    )
    parser.add_argument(
        "--model", required=True, help="The model to test (e.g., 'qwen3-1.5')."
    )
    parser.add_argument(
        "--mode", required=True, choices=["stress", "qps"], help="The test mode to run."
    )
    parser.add_argument(
        "--streaming", action="store_true", help="Enable streaming mode for responses."
    )
    ...
    for key, value in test_config.items():
        if key in config and isinstance(config[key], dict) and isinstance(value, dict):
            for inner_key, inner_value in value.items():
                if (
                    inner_key in config[key]
                    and isinstance(config[key][inner_key], dict)
    ...
    if args.model not in vendor_config["model_config"]:
        raise ValueError(
            f"Model '{args.model}' not found for vendor '{args.vendor}' in {base_config_path}"
        )
    model_config = vendor_config["model_config"][args.model]

    if args.feature not in config["features"]:
        raise ValueError(
            f"Feature '{args.feature}' not found in {base_config_path} or {args.test_config}"
        )
    ...
    if not api_key:
        raise ValueError(
            f"API key environment variable '{vendor_config['api_key_env']}' not set."
        )
    ...
    if args.mode == "stress":
        ...
        prompts = [
            p
            async for p in load_prompts(config["datasets"], feature_config["datasets"])
        ]
        ...
        for level in levels:
            ...
            writer_task = asyncio.create_task(
                results_writer(
                    results_queue,
                    output_file,
                    config["test_config"]["per_request_metrics"],
                )
            )
            ...
            print(
                f"INFO: Finished stress test for concurrency level {level}. Results saved to {output_file}"
            )
